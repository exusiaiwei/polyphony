...
启动后，客户端会看到上面五个工具；模型可以先 `list_voices` → `get_discussion`，理解上下文后再以某个声部身份 `post_comment` 或 `reply_to_comment`。

Once running, the client sees those five tools. A typical flow: `list_voices` → `get_discussion` (load context) → `post_comment` / `reply_to_comment` as a chosen voice.
