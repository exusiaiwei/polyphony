...
Polyphony is **not** "yet another AI-bot framework where you plug in an API key":
...
- ✅ The compute behind each persona comes from **the subscription AI clients you already use**: Claude Code, GitHub Copilot, Cursor, Cherry Studio, etc.
...
A typical session looks like this: in Claude Code you say "read Discussion #12 and reply as Claude" — the client uses your subscription's compute to understand the context, then calls Polyphony's MCP tools to post a comment under the *Claude* GitHub App identity. Next turn you switch to Cursor and have it speak as *GPT*. **Each persona's compute comes from a subscription you already pay for; Polyphony introduces no new model costs.**
...
spec/                         Protocol spec — defines Persona, Turn, Voice, Quote, Termination
mcp-server/                   Reference MCP server: read discussions, post comments as a chosen persona
examples/                     Configuration samples
```

Planned:

- `templates/github-app/`: per-persona GitHub App registration guides and prefill templates.
...
- `mcp-server/`: runnable minimal MCP server with `list_personas` / `list_discussions` / `get_discussion` / `post_comment` / `reply_to_comment`.
...
- **Personas upgraded to GitHub Apps**: each voice becomes a dedicated GitHub App (with the `[bot]` suffix, scoped permissions, and prefillable registration URLs), replacing the PAT scheme.
- **Semi-automated onboarding tools**: the MCP server will expose `check_setup` / `get_setup_instructions` / `validate_persona` / `add_persona` / `try_post`, so AI clients can step a user through configuration following `POLYPHONY.md`.
...
4. Once set up, future sessions are as simple as: "Read Discussion #N and reply as \<persona-id\>."
...
Your client can now read discussions and post as a chosen persona.
...
4. **AI-readable first**: the protocol itself (`POLYPHONY.md`) is written for AI assistants, so onboarding doesn't depend on any specific client's private capabilities.
5. **Small and demonstrable**: keep the reference implementation transparent enough to fork and rebuild.
